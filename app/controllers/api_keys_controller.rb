class ApiKeysController < ApplicationController
  ...
  def new
    @api_key = ApiKey.new
    @api_key.expiration = DateTime.now + 1.months
    @api_key.token = @api_key.generate_token
  end
  ...
  # POST /api_keys.json
  def create
    @api_key = ApiKey.new(api_key_params.merge(user: current_user))
  ...
  private
    ...
    def api_key_params
      params.require(:api_key).permit(:name, :token, :expiration)
    end
end
